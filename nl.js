...
psychoJS.start({
  ...
  resources: [
    // resources:
    {name: 'numberline/nl/nlprac.csv', path: 'nl/nlprac.csv'},
    {name: 'numberline/nl/nltest.csv', path: 'nl/nltest.csv'},
  ]
});
...
async function experimentInit() {
  ...
  numberline = new visual.Slider({
    ...
    size: [0.95, 0.01], pos: [0, (- 0.2)], ori: 0.0, units: psychoJS.window.units,
  ...
  instruction_3 = new visual.TextBox({
    win: psychoJS.window,
    name: 'instruction_3',
    text: '恭喜你！你已完成數字比較部分之測試。\n請按 “OK” 按鈕離開頁面，並前往下一部分。\n',
    placeholder: 'Type here...',
    font: 'Noto Sans TC',
    pos: [0, 0], 
    draggable: false,
  ...
  ok_button_3 = new visual.ButtonStim({
    win: psychoJS.window,
    name: 'ok_button_3',
    text: 'OK',
    font: 'Noto Sans TC',
  ...
  ok_button_3.clock = new util.Clock();
...
function learningLoopBegin(learningLoopScheduler, snapshot) {
  return async function() {
    ...
    learning = new TrialHandler({
      ...
      trialList: 'numberline/nl/nlprac.csv',
...
function testloopLoopBegin(testloopLoopScheduler, snapshot) {
  return async function() {
    ...
    testloop = new TrialHandler({
      ...
      nReps: 1, method: TrialHandler.Method.SEQUENTIAL,
      extraInfo: expInfo, originPath: undefined,
      trialList: 'numberline/nl/nltest.csv',
...
function preRoutineEachFrame() {
  return async function () {
    ...
    // Check numberline for response to end Routine
    if (numberline.getRating() !== undefined && numberline.status === PsychoJS.Status.STARTED) {
      continueRoutine = false; }
...
function preRoutineEnd(snapshot) {
  return async function () {
    ...
    psychoJS.experiment.addData('numberline.rt', numberline.getRT());
...
function testRoutineEachFrame() {
  return async function () {
    ...
    // Check numberline_2 for response to end Routine
    if (numberline_2.getRating() !== undefined && numberline_2.status === PsychoJS.Status.STARTED) {
      continueRoutine = false; }
...
function testRoutineEnd(snapshot) {
  return async function () {
    ...
    psychoJS.experiment.addData('numberline_2.rt', numberline_2.getRT());
...
function thankRoutineBegin(snapshot) {
  return async function () {
    ...
    // update component parameters for each repeat
    // reset ok_button_3 to account for continued clicks & clear times on/off
    ok_button_3.reset()
    ...
    thankComponents.push(ok_button_3);
...
function thankRoutineEachFrame() {
  return async function () {
    ...
    // *ok_button_3* updates
    if (t >= 0 && ok_button_3.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      ok_button_3.tStart = t;  // (not accounting for frame time here)
      ok_button_3.frameNStart = frameN;  // exact frame index
      
      ok_button_3.setAutoDraw(true);
    }
    
    
    // if ok_button_3 is active this frame...
    if (ok_button_3.status === PsychoJS.Status.STARTED) {
    }
    
    if (ok_button_3.status === PsychoJS.Status.STARTED) {
      // check whether ok_button_3 has been pressed
      if (ok_button_3.isClicked) {
        if (!ok_button_3.wasClicked) {
          // store time of first click
          ok_button_3.timesOn.push(ok_button_3.clock.getTime());
          // store time clicked until
          ok_button_3.timesOff.push(ok_button_3.clock.getTime());
        } else {
          // update time clicked until;
          ok_button_3.timesOff[ok_button_3.timesOff.length - 1] = ok_button_3.clock.getTime();
        }
        if (!ok_button_3.wasClicked) {
          // end routine when ok_button_3 is clicked
          continueRoutine = false;
          
        }
        // if ok_button_3 is still clicked next frame, it is not a new click
        ok_button_3.wasClicked = true;
      } else {
        // if ok_button_3 is clicked next frame, it is a new click
        ok_button_3.wasClicked = false;
      }
    } else {
      // keep clock at 0 if ok_button_3 hasn't started / has finished
      ok_button_3.clock.reset();
      // if ok_button_3 is clicked next frame, it is a new click
      ok_button_3.wasClicked = false;
    }
    // check for quit (typically the Esc key)
...
function thankRoutineEnd(snapshot) {
  return async function () {
    ...
    psychoJS.experiment.addData('ok_button_3.numClicks', ok_button_3.numClicks);
    psychoJS.experiment.addData('ok_button_3.timesOn', ok_button_3.timesOn);
    psychoJS.experiment.addData('ok_button_3.timesOff', ok_button_3.timesOff);
    // the Routine "thank" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset();
